...
// Models with OverwriteModelError prevention
const modelCache = {};
...
app.get('/api', (req, res) => {
  res.json({ message: 'NexGen Exam Portal API', version: '2.0' });
});
...
// -------------------- Students (Admin only except login) --------------------
...
// -------------------- Student Test Flow (No admin auth required) --------------------
...
app.post('/api/admin/pause-test', verifyAdminToken, async (req, res) => {
  try {
    ...
    if (password !== process.env.PAUSE_PASSWORD) return res.status(403).json({ error: 'Invalid pause password' });
...
app.post('/api/admin/resume-test', verifyAdminToken, async (req, res) => {
  try {
    const { studentId, testId, password } = req.body;
    if (password !== process.env.RESUME_PASSWORD) return res.status(403).json({ error: 'Invalid resume password' });
    const result = await Result.findOne({ studentId, testId });
    if (!result) return res.status(404).json({ error: 'Result not found' });
...
app.get('/api/results', verifyAdminToken, async (req, res) => {
  try {
    const results = await Result.find().sort({ submittedAt: -1 }).lean();
    res.json(results);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/results/student/:studentId', async (req, res) => {
  try {
    const results = await Result.find({ studentId: req.params.studentId }).sort({ submittedAt: -1 }).lean();
    res.json(results);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/results/test/:testId', verifyAdminToken, async (req, res) => {
  try {
    const results = await Result.find({ testId: req.params.testId }).sort({ score: -1 }).lean();
    res.json(results);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// -------------------- Discussions (Student can view, Admin manage) --------------------
...
app.post('/api/admin/community/ban', verifyAdminToken, async (req, res) => {
  try {
    ...
    if (existing) return res.status(400).json({ error: 'Student already banned' });
...
app.get('/api/admin/live-students', verifyAdminToken, async (req, res) => {
  try {
    ...
    res.json({ count: studentIds.length, students });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/settings/password', verifyAdminToken, async (req, res) => {
  res.json({ success: true, message: 'Password update simulated' });
});
