...
async function startTest(testId) {
  ...
  try {
    ...
    const [test, questions] = await Promise.all([
      apiCall('/public/tests').then(tests => tests.find(t => t.testId === testId)),
      apiCall(`/public/questions/${testId}`)
    ]);
    state.currentTest = { testId, test, questions: test.shuffle ? shuffleArray(questions) : questions, result };
    state.testAnswers = {};
      ...
      <div class="flex-1 flex overflow-hidden">
        ...
        <div class="w-80 bg-white border-l p-4 overflow-y-auto">
          ...
          <div class="grid grid-cols-5 gap-2">${state.currentTest.questions.map((ques,idx)=>{
            const qid=ques.questionId; let cls='not-visited'; if(state.testAnswers[qid]!==undefined) cls='answered'; if(state.flaggedQuestions.has(qid)) cls='flagged'; if(idx===state.currentTest.currentIndex) cls+=' current';
...
function attachTestEventListeners(currentQ){
  const modal = document.getElementById('testModal');
  modal.querySelectorAll('input[name="mcq"]').forEach(r=>r.addEventListener('change',e=>{ state.testAnswers[currentQ.questionId]=parseInt(e.target.value); renderTestInterface(); }));
  const num=modal.querySelector('#numericalAnswer'); if(num){ num.addEventListener('input',e=>{ state.testAnswers[currentQ.questionId]=parseFloat(e.target.value)||e.target.value; }); num.focus(); }
  ...
  modal.querySelector('#flagQuestionBtn')?.addEventListener('click',()=>{ state.flaggedQuestions.has(currentQ.questionId)?state.flaggedQuestions.delete(currentQ.questionId):state.flaggedQuestions.add(currentQ.questionId); renderTestInterface(); });
  ...
  modal.querySelector('#nextBtn')?.addEventListener('click',()=>{ if(state.currentTest.currentIndex<state.currentTest.questions.length-1){ state.currentTest.currentIndex++; renderTestInterface(); }else{ showSubmitConfirmation(); } });
...
function showModal(title,body,onConfirm,confirmText='Confirm'){
  const overlay=document.createElement('div'); overlay.className='fixed inset-0 bg-black/50 flex items-center justify-center z-50';
...
function startTestTimer(){
  ...
  update(); state.timerInterval=setInterval(update,1000);
}
function startPausePolling(){
  state.pauseInterval=setInterval(async()=>{
    ...
    try{
      ...
      if(status.paused){ overlay.classList.add('flex'); overlay.classList.remove('hidden'); state.currentTest.result.totalPausedDuration=status.totalPausedDuration; }
...
function handleVisibilityChange(){ if(document.hidden&&state.currentTest){ state.tabSwitchCount++; if(state.tabSwitchCount>=3){ showToast('Tab switch limit exceeded. Submitting...','error'); submitTest(true); }else{ showToast(`Warning: Tab switch (${state.tabSwitchCount}/3)`,'error'); } } }
async function submitTest(isAuto=false){
  clearInterval(state.timerInterval); clearInterval(state.pauseInterval); clearInterval(state.autoSaveInterval);
...
function showScoreModal(result){
  const modal=document.createElement('div'); modal.className='fixed inset-0 bg-black/50 flex items-center justify-center z-50';
  ...
  document.body.appendChild(modal); modal.querySelector('.closeModalBtn').addEventListener('click',()=>modal.remove());
}
function exitTest(confirm=true){ if(confirm&&!window.confirm('Exit test?')) return; clearInterval(state.timerInterval); clearInterval(state.pauseInterval); clearInterval(state.autoSaveInterval); document.removeEventListener('visibilitychange',handleVisibilityChange); document.getElementById('testModal').classList.add('hidden'); state.currentTest=null; }
...
function formatName(full, id) { const p = full.split(' '); return `${p[0]} ${p.length>1?p[p.length-1].charAt(0)+'.':''} (${id.slice(-2)})`; }
...
window.addEventListener('beforeunload', () => { if(state.timerInterval) clearInterval(state.timerInterval); if(state.pauseInterval) clearInterval(state.pauseInterval); if(state.communityInterval) clearInterval(state.communityInterval); });
